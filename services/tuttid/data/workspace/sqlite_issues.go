...
func (s *SQLiteStore) CreateTask(ctx context.Context, task workspaceissues.Task) (workspaceissues.Task, error) {
	if err := s.ensureIssueDatabase(); err != nil {
		return workspaceissues.Task{}, err
	}
	if _, err := s.GetIssue(ctx, task.WorkspaceID, task.IssueID); err != nil {
		return workspaceissues.Task{}, err
	}
...
`, task.TaskID, task.IssueID, task.WorkspaceID, task.Title, task.Content, task.SearchText,
	...
	if err != nil {
		...
		return workspaceissues.Task{}, fmt.Errorf("create workspace issue task: %w", err)
	}
...
`, taskSelectColumns), workspaceID, issueID, taskID)
...
func (s *SQLiteStore) GetNextTaskSortIndex(ctx context.Context, workspaceID string, issueID string) (int, error) {
	if err := s.ensureIssueDatabase(); err != nil {
		return 0, err
	}
...
`, workspaceID, issueID)
	...
	if err := row.Scan(&next); err != nil {
		return 0, fmt.Errorf("get next workspace issue task sort index: %w", err)
	}
	return next, nil
}
